...
namespace oceanbase
{
  ...
  namespace sql
  {
                                               ...
                                               const bool get_new_row /* default true */)
    {
      ...
      else if (OB_FAIL(create_simple_column_expr(table_name, OLD_NEW_COL_NAME, table_id, old_new)) || OB_FAIL(create_simple_column_expr(table_name, SEQUENCE_COL_NAME, table_id, seq_no)))
      {
      ...
      else if (OB_FAIL(create_simple_column_expr(table_name, WIN_MAX_SEQ_COL_NAME, table_id, win_seq)) || OB_FAIL(ObRawExprUtils::build_common_binary_op_expr(ctx_.expr_factory_, T_OP_EQ, old_new, exprs_.str_n_, equal_old_new)) || OB_FAIL(ObRawExprUtils::build_common_binary_op_expr(ctx_.expr_factory_, T_OP_EQ, seq_no, win_seq, equal_seq_no)) || OB_FAIL(ObRawExprUtils::build_common_binary_op_expr(ctx_.expr_factory_, T_OP_AND, equal_old_new, equal_seq_no, new_row_filter)))
      {
      ...
      else if (OB_FAIL(create_simple_column_expr(table_name, WIN_MIN_SEQ_COL_NAME, table_id, win_seq)) || OB_FAIL(ObRawExprUtils::build_common_binary_op_expr(ctx_.expr_factory_, T_OP_EQ, old_new, exprs_.str_o_, equal_old_new)) || OB_FAIL(ObRawExprUtils::build_common_binary_op_expr(ctx_.expr_factory_, T_OP_EQ, seq_no, win_seq, equal_seq_no)) || OB_FAIL(ObRawExprUtils::build_common_binary_op_expr(ctx_.expr_factory_, T_OP_AND, equal_old_new, equal_seq_no, old_row_filter)))
      {
                                                      ...
                                                      const uint64_t ext_sel_flags)
    {
      ...
      const bool need_old_new_col = (ext_sel_flags & MLOG_EXT_COL_OLD_NEW) || (ext_sel_flags & MLOG_EXT_COL_DML_FACTOR);
      const bool need_sql_col = (ext_sel_flags & MLOG_EXT_COL_SEQ) || (ext_sel_flags & MLOG_EXT_COL_WIN_MIN_SEQ) || (ext_sel_flags & MLOG_EXT_COL_WIN_MAX_SEQ);
      ...
      else if (need_dml_factor_col && OB_FAIL(add_dml_factor_to_select_list(old_new_col, select_items)))
      {
        LOG_WARN("failed to add dml factor to select list", K(ret));
      }
      else if ((need_win_min_col || need_win_max_col) && OB_FAIL(add_max_min_seq_window_to_select_list(table, source_table,
                                                                                                       sequence_expr,
                                                                                                       select_items,
                                                                                                       need_win_max_col,
                                                                                                       need_win_min_col)))
                                                       ...
                                                       ObRawExpr *&win_min_expr)
    {
      ...
      if (OB_ISNULL(query_ctx = ctx_.stmt_factory_.get_query_ctx()) || OB_ISNULL(schema_guard = query_ctx->sql_schema_guard_.get_schema_guard()))
      {
        ret = OB_ERR_UNEXPECTED;
      ...
      else if (OB_FAIL(ctx_.expr_factory_.create_raw_expr(T_WINDOW_FUNCTION, win_max)) || OB_FAIL(ctx_.expr_factory_.create_raw_expr(T_WINDOW_FUNCTION, win_min)) || OB_FAIL(ctx_.expr_factory_.create_raw_expr(T_FUN_MAX, aggr_max)) || OB_FAIL(ctx_.expr_factory_.create_raw_expr(T_FUN_MIN, aggr_min)))
      {
        LOG_WARN("create window function expr failed", K(ret));
      }
      else if (OB_ISNULL(sequence_expr) || OB_ISNULL(win_max) || OB_ISNULL(win_min) || OB_ISNULL(aggr_max) || OB_ISNULL(aggr_min))
      {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected null", K(ret), K(sequence_expr), K(win_max), K(win_min), K(aggr_max), K(aggr_min));
      }
      else if (OB_FAIL(aggr_max->add_real_param_expr(sequence_expr)) || OB_FAIL(aggr_min->add_real_param_expr(sequence_expr)))
      {
        LOG_WARN("failed to add param expr to agg expr", K(ret));
      }
      else if (OB_FAIL(win_max->set_partition_exprs(unique_keys)) || OB_FAIL(win_min->set_partition_exprs(unique_keys)))
      {
    ...
    int ObMVPrinter::init()
    {
      ...
      else if (OB_FAIL(ObRawExprUtils::build_const_int_expr(ctx_.expr_factory_, ObIntType, 0, exprs_.int_zero_)) || OB_FAIL(ObRawExprUtils::build_const_int_expr(ctx_.expr_factory_, ObIntType, 1, exprs_.int_one_)) || OB_FAIL(ObRawExprUtils::build_const_int_expr(ctx_.expr_factory_, ObIntType, -1, exprs_.int_neg_one_)))
      {
        LOG_WARN("failed to build const int expr", K(ret));
      }
      else if (OB_FAIL(ObRawExprUtils::build_const_string_expr(ctx_.expr_factory_, ObVarcharType, ObString("N"), cs_type, exprs_.str_n_)) || OB_FAIL(ObRawExprUtils::build_const_string_expr(ctx_.expr_factory_, ObVarcharType, ObString("O"), cs_type, exprs_.str_o_)))
      {
      ...
      else
      {
        ...
        if (OB_FAIL(ObRawExprUtils::build_const_uint_expr(ctx_.expr_factory_, ObUInt64Type, ctx_.refresh_info_->last_refresh_scn_.get_val_for_sql(), const_last_refresh_scn)) || OB_FAIL(ObRawExprUtils::build_const_uint_expr(ctx_.expr_factory_, ObUInt64Type, ctx_.refresh_info_->refresh_scn_.get_val_for_sql(), exprs_.refresh_scn_)) || (NULL != ctx_.refresh_info_->mv_last_refresh_scn_ && OB_FAIL(ObRawExprUtils::build_const_uint_expr(ctx_.expr_factory_, ObUInt64Type, ctx_.refresh_info_->mv_last_refresh_scn_->get_val_for_sql(), exprs_.mv_last_refresh_scn_))) || (NULL != ctx_.refresh_info_->mv_refresh_scn_ && OB_FAIL(ObRawExprUtils::build_const_uint_expr(ctx_.expr_factory_, ObUInt64Type, ctx_.refresh_info_->mv_refresh_scn_->get_val_for_sql(), exprs_.mv_refresh_scn_))))
        {
                                               ...
                                               ObRawExpr *&exists_expr)
    {
      ...
      else if (OB_FAIL(ctx_.expr_factory_.create_raw_expr(T_REF_QUERY, query_ref_expr)) || OB_FAIL(ctx_.expr_factory_.create_raw_expr(is_exists ? T_OP_EXISTS : T_OP_NOT_EXISTS, exists_op_expr)))
      {
        LOG_WARN("failed to create raw expr", K(ret));
      }
      else if (OB_ISNULL(query_ref_expr) || OB_ISNULL(exists_op_expr) || OB_UNLIKELY(NULL == mlog_schema))
      {
        ret = OB_ERR_UNEXPECTED;
                                                     ...
                                                     ObIArray<ObRawExpr *> &all_conds)
    {
      ...
      else
      {
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < rowkey_column_ids.count(); ++i)
        {
          ...
          else if (OB_FAIL(create_simple_column_expr(left_table.get_object_name(), rowkey_column->get_column_name_str(), left_table.table_id_, l_col)) || OB_FAIL(create_simple_column_expr(right_table.get_object_name(), right_use_orig_sel_alias ? *orig_sel_alias : rowkey_column->get_column_name_str(), right_table.table_id_, r_col)))
          {
                                                                    ...
                                                                    ObSelectStmt &target_stmt)
    {
      ...
      else
      {
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < from_items.count(); ++i)
        {
          ...
          else if (OB_UNLIKELY(idx < 0 || idx >= target_stmt.get_table_size()) || OB_ISNULL(target_stmt.get_table_item(idx)))
          {
            ret = OB_ERR_UNEXPECTED;
                                            ...
                                            ObIArray<uint64_t> &logic_pk_ids)
    {
      ...
      ObSchemaGetterGuard *schema_guard = NULL;
      bool has_pk = false;
      if (OB_ISNULL(table_schema) || OB_ISNULL(ctx_.stmt_factory_.get_query_ctx()) || OB_ISNULL(schema_guard = ctx_.stmt_factory_.get_query_ctx()->sql_schema_guard_.get_schema_guard()))
      {
        ret = OB_ERR_UNEXPECTED;
                                            ...
                                            ObIArray<ObRawExpr *> &rowkey_exprs)
    {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < rowkey_column_ids.count(); ++i)
      {
        ...
        else if (use_orig_sel_alias && OB_FAIL(get_column_name_from_origin_select_items(source_table.table_id_,
                                                                                        rowkey_column_ids.at(i),
                                                                                        orig_sel_alias)))
        {
          LOG_WARN("failed to get column_name from origin select_items", K(ret));
    ...
    int ObMVPrinter::get_mv_rowkey_column_ids(ObIArray<uint64_t> &rowkey_column_ids)
    {
      ...
      ObSchemaGetterGuard *schema_guard = NULL;
      bool has_pk = false;
      if (OB_ISNULL(ctx_.stmt_factory_.get_query_ctx()) || OB_ISNULL(schema_guard = ctx_.stmt_factory_.get_query_ctx()->sql_schema_guard_.get_schema_guard()))
      {
        ret = OB_ERR_UNEXPECTED;
                                                           ...
                                                           ObString &mview_str)
    {
      ...
      else if (NULL != mv_refresh_scn && OB_FAIL(ObRawExprUtils::build_const_uint_expr(expr_factory, ObUInt64Type, mv_refresh_scn->get_val_for_sql(), mv_scn_expr)))
      {
        LOG_WARN("failed to build const uint expr", K(ret));
      }
      else if (NULL != table_refresh_scn && OB_FAIL(ObRawExprUtils::build_const_uint_expr(expr_factory, ObUInt64Type, table_refresh_scn->get_val_for_sql(), table_scn_expr)))
      {
        LOG_WARN("failed to build const uint expr", K(ret));
      }
      ...
      else if (OB_FALSE_IT(obj_print_params.print_origin_stmt_ = true) || OB_FALSE_IT(obj_print_params.tz_info_ = expr_factory.get_query_ctx()->get_timezone_info()))
      {
      }
                                                           ...
                                                           ObRawExpr *&marker_filter)
    {
      ...
      if (OB_UNLIKELY(marker_idx < 0 || marker_idx >= mv_def_stmt_.get_select_item_size()) || OB_ISNULL(marker_expr = mv_def_stmt_.get_select_item(marker_idx).expr_) || OB_UNLIKELY(!marker_expr->is_const_expr()) || OB_ISNULL(table))
      {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected params", K(ret), K(marker_idx), K(mv_def_stmt_.get_select_item_size()), KPC(marker_expr), K(table));
      }
      else if (OB_FAIL(create_simple_column_expr(table->get_object_name(), mv_def_stmt_.get_select_item(marker_idx).alias_name_, table->table_id_, marker_col)) || OB_FAIL(ObRawExprUtils::build_common_binary_op_expr(ctx_.expr_factory_, T_OP_EQ, marker_col, marker_expr, marker_filter)))
      {
